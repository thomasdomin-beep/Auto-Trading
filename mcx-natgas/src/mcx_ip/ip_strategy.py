...
from mcx_ip.models import CrossoverPair, StrikeRow
...
def compute_ip(premiums: dict[str, float]) -> float:
    if not premiums:
        raise ValueError("No premiums provided for IP calculation")
    return sum(premiums.values()) / len(premiums)
