...
HTML = """<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"/><title>MCX Natural Gas IP Levels</title>
...
<body><h1 id="title">MCX Natural Gas IP — levels</h1><p id="status">Loading…</p><dl id="fields"></dl>
<button id="copy-sr">Copy LHS &amp; RHS</button><button id="copy-json">Copy JSON</button>
<pre id="raw"></pre>
<script>
let latest=null;
        ...
        def do_GET(self) -> None:
            ...
            if self.path == "/api/levels":
                if not levels_file.is_file():
                    ...
                    self.end_headers()
                    self.wfile.write(b'{"success":false,"error":"run mcx-ip run-once first"}')
                    return
